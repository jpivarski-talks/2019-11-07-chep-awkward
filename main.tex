...
\begin{frame}{Composable columnar data structures}
\large
\vspace{0.25 cm}
\begin{center}
\only<1>{An array of variable-length arrays (``ragged'' or ``jagged'') can be built out of flattened contents and offsets to the beginning of each subarray.}\only<2>{Multiple levels of variable-length arrays (``doubly jagged'') can be built by using one jagged array as the content of the other.}\only<3>{More complex data structures can be built by adding columnar components, such as records with named fields, mixed data types, pointers, lazy loading\ldots}
\end{center}
